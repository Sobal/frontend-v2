...
import { GasToken } from '../interfaces/tokens';
...
export class NeonProxyRpcApi {
  ...
  async evmParams(): Promise<NeonProgramStatus> {
    return this.proxy<NeonProgramStatus>('neon_getEvmParams', []).then(
      d => d.result
    );
  }

  async gasTokenList(): Promise<GasToken[]> {
    return this.proxy<GasToken[]>('neon_getGasTokenList', []).then(
      d => d.result
    );
  }

  async nativeTokenList(): Promise<GasToken[]> {
    return this.proxy<GasToken[]>('neon_getNativeTokenList', []).then(
      d => d.result
    );
  }
}
